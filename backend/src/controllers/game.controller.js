...
// Get all games
// Get all games with pagination and performance optimizations
    ...
    (async function () {
        try {
            ...
            let filter = {
                ...
                'platforms.windows.download_link': { $ne: "" }
            };
...
export const getGamesByPlatform = function (req, res) {
    (async function () {
        try {
            ...
            let findQuery = Game.find(query);
            if (mongoose.modelNames().includes("category")) {
                findQuery = findQuery.populate("category");
            }
            const games = await findQuery.exec();
            if (!games || games.length === 0) {
...
// Get popular games with pagination and sorting
