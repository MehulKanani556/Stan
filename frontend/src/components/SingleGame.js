  ...
  return (
    <div className="flex items-center">
      {Array.from({ length: fullStars }).map((_, i) => (
        <FaStar
          key={`full-${i}`}
          className={`text-yellow-400 ${size} mx-0.5`}
        />
      ))}
      {hasHalfStar && (
        <FaStarHalfAlt className={`text-yellow-400 ${size} mx-0.5`} />
      )}
...
const InfoItem = ({ label, value, capitalize = false }) => (
  <div>
    ...
    <p className={`text-white text-sm ${capitalize ? "capitalize" : ""}`}>
      {value || "-"}
    </p>
  </div>
);
...
const AccordionItem = ({ title, children, defaultOpen = false }) => (
  <details
    className="group open:shadow-lg open:bg-black/20 transition-all"
    open={defaultOpen}
  >
    ...
    <div className="pb-5 px-4 md:px-5">{children}</div>
  </details>
);
                            ...
                            <div className="relative mt-3">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  setFinalAmount(
                                    Math.max(0, gamePrice - validatedCoins)
                                  );
                                }}
                                onBlur={(e) => {
                            ...
                            <div className="flex items-center">
                              ...
                              <div className="ms-3">
                                <div className="font-medium">
                                  {
                                    decryptData(element?.user?.name)?.split(
                                      " "
                                    )[0]
                                  }
                                </div>
                                <div className="flex items-center">
                                  {Array.from({ length: FullStar }).map(
                                    (_, i) => (
                                      <FaStar
                                        key={`full-${i}`}
                                        className="text-yellow-400 h-4 w-4 mx-0.5"
                ...
                <AccordionItem title="More Details">
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between">
                      ...
                      <p className="text-white text-base">
                        {single?.developer || "Yoyo game"}
                      </p>
                    </div>
                    <div className="flex justify-between">
                      <p className="text-base text-gray-400">Publisher</p>
                      <p className="text-white text-base">
                        {single?.publisher || "Yoyo game"}
                      </p>
                    </div>
                    <div className="flex justify-between">
